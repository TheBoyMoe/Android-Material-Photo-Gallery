package com.example.materialphotogallery.common;

public class Constants {

    public static final String LOG_TAG = "LOG";

    public static final String TABLE = "photos";
    public static final String PHOTO_ID = "photo_id";
    public static final String PHOTO_TITLE = "title";
    public static final String PHOTO_DESCRIPTION = "description";
    public static final String PHOTO_FILE_PATH = "file_path";
    public static final String PHOTO_PREVIEW_PATH = "preview_path";
    public static final String PHOTO_THUMBNAIL_PATH = "thumbnail_path";
    public static final String PHOTO_SMALL_THUMB_PATH = "small_thumb_path";
    public static final String PHOTO_FAVOURITE = "favourite";
    public static final String PHOTO_LATITUDE = "latitude";
    public static final String PHOTO_LONGITUDE = "longitude";

    public static final String PHOTO_THUMBNAIL_EXT = "_thumb.jpg";
    public static final String PHOTO_SMALL_EXT = "_small.jpg";

    public static final int ERROR_DIALOG_REQUEST = 101;

}
